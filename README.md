...
- **잡기**: 물체를 잡으면 그 물체에 맞는 손모양(SphereGrab 등)으로 자동 전환, 손 위치·회전 보정까지 정확히 반영되어 스냅됨
...
| 파일 | 수정 내용 |
|---|---|
| `Hand.cs` | grab 판정을 FixedUpdate→Update로 이동(저프레임 대응). `SnapOnAttach` 실제 처리 로직 추가. **물체를 잡을 때 `RightHand` 자신이 아니라 `objectAttachmentPoint`에 직접 부모로 붙이도록 변경** — 이래야 잡은 뒤에도 `HandVisual`이 매 프레임 갱신하는 손 모양 보정(회전 오프셋 등)을 물체가 계속 따라감. `GetTrackedObjectVelocity/AngularVelocity`를 순간값 대신 최근 프레임 평균으로 스무딩 |
| `HandVisual.cs` | 장갑 3D 모델을 인스턴스화하고, 지정한 뼈(`Target Bone Name`)가 항상 RightHand 원점에 오도록 매 프레임 재정렬 — 마커 위치가 가상 손의 어느 부위에 대응될지를 결정하는 부분. **`HoverPoint`/`ObjectAttachmentPoint`도 모델과의 상대 위치+회전 관계를 캡처해서 매 프레임 그대로 재현**(실제 부모-자식으로 만들면 손 숨김 기능과 충돌할 위험이 있어 가상으로 추적). `Visual Rotation Offset Euler`로 모델의 기본 조형 각도도 보정 가능 |
...
- `SerialGloveReceiver` → `Axis Mapping`/`Invert X,Y,Z` : MPU6050 부착 방향에 따라 다름 (현재 값: `Axis Mapping = YZX`, `Invert Y`, `Invert Z` 켜짐)
...
- `HandVisual` → `Hover Point To Align`/`Attachment Point To Align` : `HoverPoint`/`ObjectAttachmentPoint` 오브젝트를 연결해둬야 함 (비어있으면 하이라이트/파지 위치가 손 모양 보정을 못 따라감)
...
- **`Hand.AttachObject`가 물체를 `RightHand` 자신에게 붙이고 잡는 순간에만 위치를 복사하던 방식이었는데,
  ...
  `objectAttachmentPoint`에 직접 부모로 붙이도록 수정해서, 잡은 이후에도 손 모양 보정을 계속 따라가게 함
...
- **Console에 "The referenced script (Unknown) on this Behaviour is missing!" 경고가 뜨는 경우**:
  `RightHand`/`DotTracker`에 예전에 붙였다가 지운 스크립트의 빈 컴포넌트 잔재. 기능엔 영향 없음,
  Inspector에서 `Missing (Mono Script)` 컴포넌트를 찾아 Remove Component 하면 됨
