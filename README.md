...
[손 위치 + 회전]
   ↓
[Unity 가상 손]  ←──  [아두이노: 포텐셔미터(손가락 굽힘) + MPU6050(IMU)]
   ↓                        ↑ USB 시리얼 (115200 baud)
[물체 grab/throw]      [서보모터 (햅틱 피드백, 예정)]
```

- **위치/회전**: 손 여러 면(손등/손바닥/손날 등)에 AprilTag(36h11) 마커를 붙이고, 웹캠으로 인식
- **손가락 굽힘**: 아두이노에 연결된 포텐셔미터 5개(엄지~소지) 값을 시리얼로 수신
- **회전 보정**: MPU6050(IMU)이 손목 회전을 직접 담당 (ArUco 위치 트래킹과 역할 분리)
- **햅틱**: 물체를 잡으면 손가락별 서보모터로 힘 피드백 (프로토콜은 준비됨, 게임 로직 연결은 예정)
...
- 보드: **ESP32** + **MPU6050**(DMP 내장, 지자기 센서 없어서 요(yaw) 드리프트 있을 수 있음)
...
| 파일 | 역할 |
|---|---|
| `ArucoHandTracker.cs` | 웹캠/영상파일 입력 → ArUco/AprilTag 인식 → 위치 계산 → 칼만 필터 → Hand에 적용 |
| `SerialGloveReceiver.cs` | 아두이노 시리얼 수신(curl+IMU), 회전 적용, tare/햅틱 명령 전송 |
| `FingerCurlAnimator.cs` | curl 값으로 장갑 모델의 손가락 뼈를 실제로 굽힘 (Valve 레퍼런스 포즈 블렌딩) |
| `GraspPoseTrigger.cs` | 물체를 잡으면 그 물체에 맞는 손모양(SphereGrab 등)으로 전환 |
...
- `FingerCurlAnimator` → `Curl Axis` : 장갑 모델 방향에 따라 조정 필요할 수 있음
...
- `GraspPoseTrigger`를 물체에 붙일 때 `[RequireComponent(typeof(Interactable))]`를 쓰면 안 됨 —
  이 씬은 `Interactable`이 보통 부모 오브젝트에 있어서, 자식에 붙이면 중복 생성되어 잡기 자체가 꼬임.
  (`GetComponentInParent`로 찾도록 이미 수정되어 있음, 재발 방지용 기록)
